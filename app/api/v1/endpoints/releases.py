...
def list_releases(
    ...
    current_user: UserModel = Depends(check_permission("read:releases"))
) -> Any:
    """
    List all releases.
    """
...
def create_release(
    ...
    db: Session = Depends(get_db),
    current_user: UserModel = Depends(check_permission("create:releases"))
) -> Any:
    """
...
def update_release(
    ...
    if "services" in data:
        ...
        db.query(ReleaseServiceLinkModel).filter(ReleaseServiceLinkModel.release_id == id).delete()
...
@router.post("/{id}/deploy", response_model=Deployment)
def deploy_release(
    id: UUID,
...
def delete_release(
    ...
    # Let's check: DeploymentModel definition has `release = relationship(..., back_populates="deployments")`
    ...
    db.query(DeploymentModel).filter(DeploymentModel.release_id == id).delete()
    db.delete(release)
    db.commit()
    return None


@router.delete("/{id}/deploy/{environment_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.delete("/{id}/deploy/{environment_id}/{service_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.get("/{id}/report", response_class=StreamingResponse)
def generate_release_report(
    ...
    env_map = {str(e.id): e.name for e in environments}
    ...
    release_info = [
        ...
        ["Created At", release.created_at.strftime("%Y-%m-%d %H:%M:%S") if release.created_at else "—"],
        ["Planned Date", release.planned_release_date.strftime("%Y-%m-%d %H:%M:%S") if release.planned_release_date else "—"],
    ]
            ...
            for env in environments:
                deployment = next(
                    (d for d in release.deployments 
                     if str(d.environment_id) == str(env.id) and str(d.service_id) == str(link.service_id) and d.status == "success"),
                    None
                )
                if deployment:
